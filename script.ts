...
- Keep responses conversational and accessible, suitable for both beginners and experienced users.`;
  const CMC_API_KEY =
    (typeof window !== "undefined" && window.CMC_API_KEY) ||
  ...
  const introContent = document.querySelector(
    ".ai-container > .ai-message-content"
  ) as HTMLElement | null;
  ...
  function appendMessage(message: string, sender: "user" | "bot", options?: { isTyping?: boolean }) {
  ...
  async function typewriterToElement(element: HTMLDivElement, text: string) {
    if (!element) return;
    element.textContent = "";
  ...
  async function getBotResponse(userMessage) {
    ...
    if (!apiKey) {
      throw new Error(
        "Missing OpenAI API key. Set `window.OPENAI_API_KEY` before using the assistant."
      );
    }
    ...
    try {
      ...
      const change24h =
        typeof usd.percent_change_24h === "number"
          ...
          : "N/A";
      const marketCap =
        typeof usd.market_cap === "number" ? formatUsd(usd.market_cap) : "N/A";

      return `**Lyceum Assistant** live quote (CoinMarketCap):\n- **Asset:** ${data.name} (${data.symbol})\n- **Price (USD):** ${price}\n- **24h Change:** ${change24h}\n- **Market Cap:** ${marketCap}`;
    } catch (error) {
      const message =
        error && error.message ? error.message : "Unknown CMC API error";
  ...
  async function fetchFreshContext(query: string, forceFetch = false) {
    ...
    try {
      ...
      if (webRes.ok) {
        ...
        if (web && typeof web.AbstractText === "string" && web.AbstractText.trim()) {
          ...
          const firstText =
            web.RelatedTopics.find((t) => typeof t.Text === "string" && t.Text)?.Text || "";
